...
export default (gameManager) => {
	return (event) => {
		...
		if ((!gameManager.isRunning() || gameManager.isPaused()) && event.type === 'keydown') {
		...
		[
			...
			if (event.keyCode === key[4]) {
				if (event.type === 'keydown') {
					if (player.dropInterval !== player.fast()) {
						player.drop(player);	//Move down
						player.dropInterval = player.fast();
					}
				}
				else {
					player.dropInterval = player.slow();
				}
			}
		});
	}
